...
def write_txt_intro(txt, start_page):
    fo = open("txt_intro{0:0>8}_{1:0>8}.txt".format(
        start_page, start_page+100), "ab+")
    try:
...
def get_txt(txt_id, start_page):
    ...
    try:
        ...
        txt['title'] = soups.select('#wrapper .box_con #maininfo #info h1')[
            0].text  # 获取小说题目
        ...
        # print("正在寻找第一章页面。。。")
        first_page = soups.select(
            '#wrapper .box_con #list dl dd a')  # 获取小说所有章节信息
        ...
        while(1):
            try:
                ...
                txt_section = soup.select('#wrapper .content_read .box_con .bottem2 #A3')[
                    0]['href']  # 获取下一章地址
                if(txt_section == './'):  # 判断是否最后一章  最后一章则跳出循环
                    print(
                        "编号："+'{0:0>8}   '.format(txt['id']) + "小说名：《"+txt['title']+"》 下载完成")
                    break
                # 以二进制写入章节题目
...
def get_txts(start_page):
    ...
    while (1):
        ...
        if (i_ls_ct % 10 >= 4):
            ...
            print(r'{0:0>8}-{1:0>8} '.format(start_page, start_page+100)+"下载中："
                  + "*" * (int)((task_ct - run_task) / task_ct * 50) + "_" * (int)(
                run_task / task_ct * 50) + r" \. 总数：" + str(task_ct) + "  已完成：" + str(
